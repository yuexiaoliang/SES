...
def update_collected_time(name):
  ...
  db.timestamps.update_one({"name": name}, {"$set": {"collected_timestamp": time, "collected_time": utils.format_timestamp(time, '%Y-%m-%d %H:%M:%S')}}, upsert=True)
...
def get_stocks_history(stock_codes):
  ...
  # 采集的数据
  stock_history_dict = ef.stock.get_quote_history(stock_codes, beg=start_time)
  ...
  for code, value in stock_history_dict.items():
    ...
    try:
      ...
      # 把数据插入到数据库中
      db.stock_history.insert_many(utils.transform_data(stock_history_data))
